...
public class UserServeiceImpl extends ServiceImpl<UserDAO, UserEntity> implements UserService {
    ...
    public IPage<UserDTO> selectUserList(UserQueryDTO queryDTO) {
        ...
        // 排序
        queryDTO.getPageParam().getOrders().stream().forEach(orderItem -> orderItem.isAsc()?queryWrapper.orderByAsc(orderItem.getColumn()):queryWrapper.orderByDesc(orderItem.getColumn())));
