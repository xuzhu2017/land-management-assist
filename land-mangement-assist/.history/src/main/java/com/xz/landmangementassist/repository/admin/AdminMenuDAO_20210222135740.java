...
public interface AdminMenuDAO extends BaseMapper<AdminMenuEntity> {
    @Select("select t_admin_menu.* from t_admin_user_role inner join t_admin_role_menu on t_admin_user_role.role_id = t_admin_role_menu.role_id inner join t_admin_menu on t_admin_role_menu.menu_id = t_admin_menu.id where id = #{userQueryDTO.id}")
