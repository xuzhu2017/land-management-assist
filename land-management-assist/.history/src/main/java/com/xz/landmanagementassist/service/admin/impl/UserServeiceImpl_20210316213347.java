...
public class UserServeiceImpl extends ServiceImpl<UserDAO, UserEntity> implements UserService {
    ...
    public Result<UserDTO> login(UserDTO user) {
        ...
        try {
            ...
            if (!userDTO.isDeleted()) {
                return Result.error(ErrorInfoEnum.LOGIN_USERDELETE_ERROR);
            }
            return Result.success(userDTO);
        } catch (IncorrectCredentialsException e) {
            return Result.error(ErrorInfoEnum.LOGIN_PWD_ERROR);
        } catch (UnknownAccountException e) {
            return Result.error(ErrorInfoEnum.LOGIN_USERNOTFOUND_ERROR);
        }
    }
    ...
    public Result<Boolean> register(UserDTO user) {
        ...
        MyAssert.isTrue(!(user.getUsername().isEmpty() || user.getPassword().isEmpty()), "用户名和密码不能为空！");
