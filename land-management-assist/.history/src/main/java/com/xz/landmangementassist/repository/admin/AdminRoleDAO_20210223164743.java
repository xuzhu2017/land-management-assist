...
public interface AdminRoleDAO extends BaseMapper<AdminRoleEntity> {
     ...
     * @return
     */
    @Select("<script>" + "select t_admin_permission.*,t_admin_role_permission.role_id  " + "from t_admin_permission "
            + "inner join t_admin_role_permission on t_admin_permission.menu_id = t_admin_role_permission.id "
            + "where t_admin_role_menu.role_id in ("
            + "<foreach item='id' collection='roleIdList' open='(' separator=',' close=')'>" + "#{id}" + "</foreach>"
            + ")" + "</script>")
